classdef OptiBatchQNPUR < Opti
    ...
    % Date: 1/13/2025.
    % the gradient is uniformly sampling at each iteration.
    % **************************
    ...
    methods
        %% Constructor
        function this = OptiBatchQNPUR(F,D,reg,TVtype,bounds,OutOp)
            if nargin < 6 || isempty(OutOp), OutOp = OutputOpti(1);end
            if nargin < 5 || isempty(bounds), bounds = [-inf,inf];end
            this.name = 'OptiQNP_UR';
            this.F = F;
            this.F.counterSet = false;
            this.F_temp = F;
            this.D = D;
        ...
        function flag = doIteration(this)
            ...
            if this.niter<this.Numblock
                ...
                this.F.counterSet = true;
                this.F.counterGiven = randsample(-1:this.Numblock-2,1);
                indoi = this.F.counterGiven+2;
            end

            g_k_1 = this.F.applyGrad(this.xopt); % ** Tao: it is better we can add one option to select the subset. **
            this.xold = this.xopt;
            
            % first iteration // only need to formulate B^{-1}
            if  this.niter == 0 || this.niter< this.Numblock
        ...
        function [D_diag,u,D_inv,u_inv] = EstimateHessian(this,s_k,y_k)
            ...
            % y_k: diff \nabla f(x+k)
            %  estimate Hessian
            % formulate Hessian. *** use matrix form ***
            ...
            switch this.UpdateHessianType
                case 'SRI'
                    ...
                    if tau_BB2<0
                        D_inv = 1/(this.a_k*this.L)*ones_(1);
                        D_diag = 1/D_inv;
                        u_inv = zeros_(this.sizein);
                        u = zeros_(this.sizein);
                    else
        ...
        function v_out = ProxTVWarm_NV(this,v)
            ...
            % -------------------------------------------------------------------------
            % looks we need to add *this.Numblock since the data fidelity is accumulated for block version
            ...
            for index_Hessian = 1:this.Numblock
                %D_acc = D_acc+this.Hessian_D_k_cell(index_Hessian);
            ...
            %sigma_max = 3/4*sigma_max; % One can set larger stepsize - Ref. Results of my new paper. or set 3/4 to be 1 as we used before.
            ...
            for iterin = 1:this.maxiterin
                ...
                if count>1 % we have no-zeros u.
                    ...
                    %xxx = fsolve(Eval_fun,x0,this.options);
                    iter_count = 0;
                    xxx = x0;
                    %grad_norm_set = [];
                    while 1
                        ...
                        grad = Eval_fun(xxx);
                        %grad_norm_set = [grad_norm_set;norm(grad)];
        ...
        function [lambda,b] = Power_Iteration(this,Ax,size_in,tolerance)
            ...
            if nargin <=3
                tolerance = 1e-4;%changed 15/09/2022 by TAP
            end
        ...
        function [x,a,cnt] = prox_rank1_generic_Tao(this,prox,prox_brk_pts,x0,D,u,uinv,lambda,linTerm, plusminus,INVERT)
            ...
            %brk_pts = sort(brk_pts(:)); brk_pts(brk_pts((1:end-1)') == brk_pts((2:end)')) = [];
