%% MAIN 3D Born
fpath = createFolder(par.folder,'LSSimbornReco',par.param_fname);%copy main_param in (newly created) folder
...
for ll = 1:length(algName_set)
    ...
    for kk = 1:length(regul_set) 
        for JJ = 1:length(Numsubset)
            ...
            % par.regul = regul_set(kk)/10; % for simulated data.
            fprintf('Starting regul %1.2e, gam %1.2e, Niterin %i\n',par.regul,par.gam,par.Niterin);

            OutOp = myOutputOpti(1,f_res.born,par.iterVerb,[par.k*par.dz,simP.k],simP.n0,'born',ceil(par.siz/2));
            OutOp.doPlot = false;
