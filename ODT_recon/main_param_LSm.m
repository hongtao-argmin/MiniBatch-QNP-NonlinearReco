...
par.excluded_angles = [];
par.Ntheta = simP.Ntheta;%-1
par.noreflection = 0;

par.SP = SP;
%% Algo parameters (FISTA or ADMM for complex and intensity rsp.)
par.Lsub = 12; %Number of angles considered for gradient evaluation (stochastic or not)
par.stochGrad = 2;
...
par.Niterin = 200;%number of iterations for iterative forward

%Intensity only INNER FISTA
par.NiterInnerFISTA = 200;
...
par.prox_iter = 100;%number of iterations for ADMM
...
par.uin = 'Soulez';%'sim';%'real';%'cylindrical';%'planewave';

%ONLY used when par.uin == 'real'
%par.distUin = -par.Lx/2 + par.dz*20;%BPM
par.shift = -simP.dz;
%par.distUin = -par.Lz/2;% - par.dx;%BPM
par.distUin = max(unique(SP.z)) - simP.Lz/2 + (1:par.Nz)*par.dz + par.shift - par.Lz/2;%others
%Data measurements

par.sigma = 0;
uM.curr_y = uM.GTs;%or any suitable variable

%Prop = LinOpTiltedPropagator(simP.lambda0,simP.n0,0,par.dx,[simP.Nx,simP.Ny],[0,0],'AS');
curr_uem = uems;
%for kk = 1:simP.Ntheta
%    uM.curr_y(:,:,kk) = Prop*uM.curr_y(:,:,kk);
%    curr_uem(:,:,kk) = Prop*curr_uem(:,:,kk);
%end
...
par.distPropShift = -par.Lz/2 - par.dz;
...
par.snr = psnr(abs(uM.GTs - uem),abs(uM.curr_y - uem));
...
%regul_set = [150,200,250,500,100];
% for my simulated one
% regul_set = 1e-1;%[1;1.5;2];%1e-4;1e-3;5e-3;1e-2;1e-1;6e-1;1e-4;1e-3;[1e-2;1;5;10];%[10,15,25,30];%,7,8[1,5];%,
% regul_set = regul_set/prod(par.siz);
% par.gam = 4e1;%Step size for FBS (FISTA)
%%
% for shepp one
regul_set = 1;%[5e-3;1e-2;5e-2;1e-1;1;1.5;2;10];%1e-4;1e-3;5e-3;1e-2;1e-1;6e-1;1e-4;1e-3;[1e-2;1;5;10];%[10,15,25,30];%,7,8[1,5];%,
regul_set = regul_set/prod(par.siz);
par.gam = 2e1;
