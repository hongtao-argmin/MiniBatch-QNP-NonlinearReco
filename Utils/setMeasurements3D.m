...
if ~isplanar
    ...
    roix_brut = [ceil(simP.Lxroi/simP.dx/2),floor(simP.Lxroi/simP.dx/2)];
    roiy_brut = [ceil(simP.Lyroi/simP.dy/2),floor(simP.Lyroi/simP.dy/2)];
    if isfield(simP,'roi')
        roi_center = simP.roi;
    else
    ...
    if par.SP.nCamera==1
        out = out(max(1 + roi_center(1) - roiy_brut(1),1):roi_center(1) + roiy_brut(2),...
            max(1 + roi_center(2) - roix_brut(1),1):roi_center(2) + roix_brut(2),:);
    ...
    if hasUin
        if par.SP.nCamera==1
            uinin = uinin(max(1 + roi_center(1) - roiy_brut(1),1):min(roi_center(1) + roiy_brut(2),end),...
                max(1 + roi_center(2) - roix_brut(1),1):min(roi_center(2) + roix_brut(2),end),:);
        elseif par.SP.nCamera==2
            if noreflection
                uinin = uinin(max(1 + roi_center(1) - roiy_brut(1),1):roi_center(1) + roiy_brut(2),...
                    max(1 + roi_center(2) - roix_brut(1),1):roi_center(2) + roix_brut(2),2,:);
            else
                uinin = uinin(max(1 + roi_center(1) - roiy_brut(1),1):roi_center(1) + roiy_brut(2),...
                    max(1 + roi_center(2) - roix_brut(1),1):roi_center(2) + roix_brut(2),:,:);
            end
    ...
    if FFTGtild %lipp(i),rytj(i) use Gtild
        ...
        if strcmpi(algo,'lipp') || strcmpi(algo,'bpmi')
    ...
    if FFTGtild && ~(nargin > 8 && strcmpi(varargin{2},'noproc')) || (~FFTGtild && par.dx~=simP.dx)
        if par.SP.nCamera==1
            out = imresize3(out,[par.SP.Nmeas,par.Ntheta],methrsz);
            if nargout > 2
                varargout{1} = imresize3(varargout{1},[par.SP.Nmeas,par.Ntheta],methrsz);
            end
        elseif par.SP.nCamera>=2
            if strcmpi(algo,'bpm')
                out = imresize3(out,[par.Nx,par.Ny,2 - noreflection,par.Ntheta],methrsz);
            else
                out = imresize3(out,[par.SP.Nmeas,2 - noreflection,par.Ntheta],methrsz);
            end
            if nargout > 2
                varargout{1} = imresize3(varargout{1},[par.SP.Nmeas,2 - noreflection,par.Ntheta],methrsz);
            end
        end
    end
end
