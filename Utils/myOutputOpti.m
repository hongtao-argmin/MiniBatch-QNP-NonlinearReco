classdef myOutputOpti < handle
    ...
    methods
        ...
        %% Update method
        function update(this,opti,TaoTime)
            this.PerIterTimer(this.count) = TaoTime;
            this.nTimer  = cumsum(this.PerIterTimer);
            % if this.count == 1
            %     this.nTimer(this.count) = TaoTime;
            % else
            %     this.nTimer(this.count) = this.nTimer(this.count)+TaoTime;%toc(this.start);
            % end
            ...
            if this.computecost
                ...
                switch opti.name
                    ...
                    case {'Opti FBS','Opti FBS SMLM','my Opti FBS','OptiWeightedProx'}
                        ...
                        %else
                        % ****
                        % Tao changed that we are able to calculate the
                        % whole cost. :) ***

                        temp_aa = opti.F;
                        temp_aa.setPartialGrad(0) 
                        this.evolcost(this.count,1) = gather(temp_aa.apply(opti.xopt));
                        temp_aa.setPartialGrad(2) 
                        %opti.F.apply(opti.xopt);
                        % ***
                        ...
                        %                        end
                        if strcmp(opti.name,'OptiWeightedProx')
                            this.evolcost(this.count,2) = gather(opti.cost.mapsCell{2}.apply(opti.xopt));
                        else
                            this.evolcost(this.count,2) = gather(opti.G.apply(opti.xopt));
                        end
                        this.evolcost(this.count,3) = norm(opti.xopt-opti.xold,'fro');
                    ...
                    case 'Opti IncADMM'
                        this.evolcost(this.count,1) = 0;
                        for nn = 1:length(opti.Lsub)
                            kk = opti.subset(nn);
                            this.evolcost(this.count,1) = this.evolcost(this.count,1) ...
                                + gather(opti.Fn{kk}.apply(opti.Hn{kk}*opti.xopt));
                        end
                        if ~isempty(opti.F0)
                            this.evolcost(this.count,2) = gather(opti.F0.apply(opti.xopt));
                        end
                    case 'OptiVMLMB'
                ...
                if size(this.evolcost,2) > 1% Tao corrected
                    for kk = 2:2%size(this.evolcost,2)
                    ...
                    str = sprintf('%s = %4.4e',str,sum(this.evolcost(this.count,1:2)));
                end
                %else
            ...
            % move above the estimate the cost.
            ...
            if (mod(opti.niter,this.iterVerb)==0) || opti.niter==1
                ...
                if this.doPlot% && this.iternum(min(this.count,end))>=50
                    ...
                    if ismatrix(opti.xopt)
                        ...
                        if ~isempty(this.xtrue)
                            if contains(this.optiType,'BPM')
                                cax = [min(this.ntrue(:)),max(this.ntrue(:))] - this.n0;
                            else
