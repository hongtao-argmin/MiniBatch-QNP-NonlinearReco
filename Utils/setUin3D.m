...
switch par.uin
    ...
    case {'real','Soulez','realint'}
        if strcmp(par.uin,'Soulez') || strcmp(par.uin,'realint')
            %%
            ...
            if hasandis(par,'doUinApod')
                if iscell(simP.Ltheta)
                    fname = sprintf('%s_uin_%s_%i_%i_%1.2f_Ntheta_%i_sz_%i%i%i_szuinin_%i_%i_dx_%1.2e_ltheta_%f_%f.mat',...
                        simP.ObjectType,algo,pfact,Napod,t,par.Ntheta,par.siz,sz_uinin,simP.dx,simP.Ltheta{1});%,simP.Ltheta(1));%
                else
                    fname = sprintf('%s_uin_%s_%i_%i_%1.2f_Ntheta_%i_sz_%i%i%i_szuinin_%i_%i_dx_%1.2e_ltheta_%f_%f.mat',...
                        simP.ObjectType,algo,pfact,Napod,t,par.Ntheta,par.siz,sz_uinin,simP.dx,simP.Ltheta(1));%,simP.Ltheta{1});%
                end
                ...
                if iscell(simP.Ltheta)
                    fname = sprintf('%s_uin_%s_%i_Ntheta_%i_sz_%i%i%i_szuinin_%i_%i_dx_%1.2e_ltheta_%f_%f.mat',...
                        simP.ObjectType,algo,pfact,par.Ntheta,par.siz,sz_uinin,simP.dx,simP.Ltheta{1});%,simP.Ltheta(1));%
                else
                    fname = sprintf('%s_uin_%s_%i_Ntheta_%i_sz_%i%i%i_szuinin_%i_%i_dx_%1.2e_ltheta_%f_%f.mat',...
                        simP.ObjectType,algo,pfact,par.Ntheta,par.siz,sz_uinin,simP.dx,simP.Ltheta(1));%,simP.Ltheta{1});%
                end
            end
            if isfield(simP,'specklelevel')
                fname = sprintf('%s_speckle_%1.2f.mat',fname(1:end-4),simP.specklelevel);
            end
            fname = sprintf('%s_distUin_%1.2f_%1.2f.mat',fname(1:end-4),par.distUin(1),par.distUin(end));
            if exist(fname,'file') && nargout == 1
                load(fname);
            else
                if ~isfield(simP,'roi')
                    simP.roi = sz_uinin/2;
                ...
                yy = (simP.dx:simP.dx:(sz_uinin(1)*simP.dx)) - (sz_uinin(1)*simP.dx)/2 - 0*simP.dx;
                xx = (simP.dy:simP.dy:(sz_uinin(2)*simP.dy)) - (sz_uinin(2)*simP.dy)/2 - 0*simP.dx;
                [XXB,YYB] = meshgrid(xx,yy);
                if nargout > 1
                    varargout{1} = zeros([sz_uinin,par.Ntheta]);
                end
                global isGPU
                tmp_isGPU = isGPU;
                %useGPU(0);
                for ll = 1:par.Ntheta
                    ...
                    if strcmp(par.uin,'realint')
                        uinin(:,:,par.curr_ind_thetas(ll)) = uinin(:,:,par.curr_ind_thetas(ll)).*tiltFactorB;
                    end
                    curr_uinf = fft2(Padder*(bounwin.*uinin(:,:,par.curr_ind_thetas(ll))./tiltFactorB));
                    cthetas = rad2deg(par.thetas(ll,[2,1]))';%asin(simP.k*sin(par.thetas(ll,[2,1]))*simP.lambda0/(2*pi).*[simP.Lx,simP.Ly])';
                    P = LinOpTiltedPropagator(simP.lambda0,...
                        simP.n0,0,simP.dx,pfact*sz_uinin,-cthetas,'AS');
                    lf = @(z) 1;%@(z) abs(z*sqrt(P.mu)./(sqrt((  P.n0/ P.lambda)^2- (P.mu + P.mv)))) <= P.sizein(1)*P.dxy...
                    ...
                    if nargout >1
                        varargout{1}(:,:,ll) = uinin(:,:,par.curr_ind_thetas(ll));
                    end
                    toc
                end
                if nargout==1
