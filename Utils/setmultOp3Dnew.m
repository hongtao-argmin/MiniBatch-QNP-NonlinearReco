...
switch algo
    case {'lipp','lippi','rytj','rytji','born','borni',...
            'seagle','seaglei','bpm','helm','lippSMLM'}
...
for kk = 1:Ntheta
    switch algo
         case {'born','borni'}
            if iscell(Gtil)
                out{kk} = LinOpBorn3D(Gtil{ind_shared_sensors(kk)},M,uin(:,:,:,kk),uem(:,kk),...
                    strcmpi(algo,'borni') | ~par.yscat);
            else
                out{kk} = LinOpBorn3D(Gtil,M,uin(:,:,:,kk),uem(:,:,kk),...
                    strcmpi(algo,'borni') | ~par.yscat);
            end
            ...
            if ~iscell(Gtil) && any(M.sizeout - out{kk}.sizeout)
                out{kk} = LinOpSelectorPatch(out{kk}.sizeout, 1 + floor((out{kk}.sizeout - M.sizeout)/2),...
                    floor((out{kk}.sizeout + M.sizeout)/2))*out{kk};
            end
        case {'rytj','rytji'}
            if iscell(Gtil)
